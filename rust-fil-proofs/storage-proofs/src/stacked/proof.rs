...
impl<'a, H: 'static + Hasher, G: 'static + Hasher> StackedDrg<'a, H, G> {
    ...
    pub(crate) fn prove_layers(
        ...
        println!("prove_layers begin:  graph ={:?}",graph);
        ...
        println!("t_aux = {:?}",t_aux);
        println!("_total_layers = {:?}   layerIndex = {}",_total_layers,layers);
        //for i in 1..layers+1 {
           // let lables0 = t_aux.labels_for_layer(i);
        println!("t_aux.labels.len() = {:?}",t_aux.labels.len());           
       // println!("t_aux columns = {:?}",t_aux.labels.columns());
        ...
        let get_drg_parents_columns = |x: usize| -> Result<Vec<Column<H>>> {
            ...
            for parent in &parents {
                columns.push(t_aux.column(*parent)?);// col -> label
            }

            debug_assert!(columns.len() == base_degree);

            //println!("base parent columns of index:  {} =  {:?}",x, columns);
            Ok(columns)
        };
            ...
            .map(|k| {
                ...
                challenges
                    ...
                    .map(|(challenge_index, challenge)| {
                        println!(" challenge {} (index={})", challenge, challenge_index);
                        ...
                        let aproof = t_aux.tree_d.gen_proof(challenge)?;
                        //println!("aproof  = {:?}", aproof);
                        //proof 到 merkleproof是从叶子到root的顺序调转,proof:叶子到根  merkleproof是root到leaf
                        let comm_d_proof =
                            MerkleProof::new_from_proof(&aproof);
                        //println!("comm_d_proof from aproof  = {:?}", comm_d_proof);
                        ...
                        //println!("ReplicaColumnProof  = {:?}", rpc);

                        // Final replica layer openings
                        println!("final replica layer openings");
                        ...
                        //println!("comm_r_last_proof  = {:?}", comm_r_last_proof);
                        ...
                        for layer in 1..=layers {
                            println!("encoding proof layer {}", layer);
                            ...
                            //println!("parents_data.len()  = {:?}", parents_data.len());
                            //println!("parents_data  = {:?}", parents_data);
                            ...
                            //println!("parents_data_full.len()  = {:?}", parents_data_full.len());
                            //println!("parents_data_full  = {:?}", parents_data_full);
    ...
    pub(crate) fn transform_and_replicate_layers_inner(
        ...
        println!("transform_and_replicate_layers");
        let nodes_count = graph.size();

        //图的节点数*NODE_SIZE应该等于数据长度
        assert_eq!(data.len(), nodes_count * NODE_SIZE);
        println!("nodes count {}, data len {}", nodes_count, data.len());
        ...
        //生成列哈希，然后生成tree_c
        //把数据分成chunk，然后分多线程处理，根据层生成列哈希，写入hashs这个vec，然后再使用vec生成treec
        let tree_c = measure_op(GenerateTreeC, || {
            println!("Building column hashes");
            ...
            rayon::scope(|s| {
                ...
                //println!("hashes.chunks_mut 1= {:?}",hashes.chunks_mut(1).enumerate());
                //println!("hashes.chunks_mut 2= {:?}",hashes.chunks_mut(2).enumerate());
                ...
                for (chunk, hashes_chunk) in hashes.chunks_mut(chunk_size).enumerate() {
                    ...
                    //println!("chunk = {:?}",chunk);
                    s.spawn(move |_| {
                        for (i, hash) in hashes_chunk.iter_mut().enumerate() {  //段中单个单个处理
                            //println!("i is column index = {:?}",i); //整个分段数组中的索引
        ...
        //用最后一层的label和原始数据分块合并哈希，使用结果生成默克尔树
        println!("building tree_r_last");
        let tree_r_last = measure_op(GenerateTreeRLast, || {
            data.ensure_data()?;
            println!("labels = {:?}",labels);
            let last_layer_labels = labels.labels_for_last_layer()?;
            let size = Store::len(last_layer_labels);
            println!("last_layer_labels size = {:?}  NODE_SIZE={}",size,NODE_SIZE);  //not same storesize
            let encoded_data = last_layer_labels
                .read_range(0..size)?  //[u8: 32] -> Vec<H::Domain()>     Result<Vec<E>> 
                .into_par_iter()
        ...
        println!("tree d len {}", tree_d.len());
        println!("tree c len {}", tree_c.len());
        println!("tree r_last len {}", tree_r_last.len());
