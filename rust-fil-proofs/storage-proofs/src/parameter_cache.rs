    ...
    P: ParameterSetMetadata,
{
    ...
    fn get_groth_params(circuit: C, pub_params: &P) -> Result<groth16::MappedParameters<E>> {
        println!("get_groth_params pre start");
        let id = Self::cache_identifier(pub_params);
        println!("get_groth_params start");
        ...
        let cache_result = read_cached_params(&cache_path);
        println!("cache_result finish");
        match  cache_result {
            Ok(x) =>  {
                println!("read_cached_params OK!");
                Ok(x)
            },
            Err(_) => {
                let gen = generate()?;//运算时间很长，所以需要第一次生成后缓存到硬盘上
                println!("generate() finish");
                write_cached_params(&cache_path, gen).unwrap_or_else(|e| {
                    println!("{}: failed to write generated parameters to cache", e);
                    panic!("{}: failed to write generated parameters to cache", e)
                });
                println!("read_cached_params fail!");
