    ...
    fn graph_bucket<H: Hasher>() {
        ...
        for size in vec![4, 16, 256/*, 2048*/] {
            ...
            for i in 2..size {
               ...
               // println!("node's parents   {:?} ---> {:?}",i,p1);
                let mut p2 = vec![0; degree];
                g.parents(i, &mut p2).unwrap();
               // println!("node's parents   {:?} ---> {:?}",i,p2);
